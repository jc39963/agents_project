...
def find_similar_items(query, top_k: int = 5):
    ...
    for match in results.matches:
        print(
            f"ID: {match.id} | Score: {match.score:.4f} | URL: {match.metadata.get('product_url')}"
        )
        output["matches"].append(
            {"id": match.id, "score": float(match.score), "metadata": match.metadata}
        )
...
TOOL_SCHEMAS = [
    {
        ...
        "function": {
            ...
            "description": (
                "returns embedding from fashion-CLIP model of inputted image, always call this first and then use output embedding to decide what clothes to pair with."
            ),
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Takes input text or query, encodes the text into an embedding, and searches Pinecone vector database for "
    ...
    },
    {
        ...
        "function": {
            "name": "encode_images",
            "description": (
                "Takes input list of image paths, uses CLIP model to turn images into embeddings and returns"
                "as a list of floats"
            ),
            "parameters": {
                "type": "object",
    ...
    },
    {
        ...
        "function": {
            "name": "encode_texts",
            "description": (
                "Takes text input, uses CLIP model to turn text into embeddings and returns embedding "
                "as a list of floats"
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    ...
                    "texts": {"type": "string", "description": "text to embed"},
                },
                "required": ["texts"],
            },
        },
...
# print(get_image_embedding("data/images/user_image.jpg"))